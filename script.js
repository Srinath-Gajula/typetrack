...
function handleInput(e) {
    ...
    const previousLength = currentIndex;   // store first
    currentIndex = inputValue.length;      // update after
    // Detect new character typed (not backspace)
    if (currentIndex > previousLength) {
        const typedChar = inputValue[currentIndex - 1];
        const correctChar = currentText[currentIndex - 1];
...
function updateStats() {
    ...
    // Correct-character-based WPM
    const wpm = Math.max(0, Math.round((correctChars / 5) / minutes));

    // Raw keystroke accuracy (your current philosophy)
    const accuracy = totalKeyPresses > 0
        ? Math.round(((totalKeyPresses - totalMistakes) / totalKeyPresses) * 100)
        : 100;
